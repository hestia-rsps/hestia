...
internal class ExtendedFastEventDispatcherTest {
    ...
    fun `Registered listener dispatches`() {
        ...
        var counter = 0
        val eventListener = ExtendedEventListener(event::class, 0, false, null) {
            counter++
        }
    ...
    fun `Registered listener dispatches in correct order`() {
        ...
        dispatcher.register(ExtendedEventListener(event::class, 2, false, null) {
            assertThat(counter++).isEqualTo(1)
        })
        //Registered last, but priority called first
        dispatcher.register(ExtendedEventListener(event::class, 10, false, null) {
            counter++
        })
        dispatcher.dispatch(event)
        //Then
