...
import worlds.gregs.hestia.network.client.decoders.messages.FloorItemOption

class FloorItemOptionDecoder : MessageDecoder<FloorItemOption>(7, FLOOR_ITEM_OPTION_1, FLOOR_ITEM_OPTION_2, FLOOR_ITEM_OPTION_3, FLOOR_ITEM_OPTION_4, FLOOR_ITEM_OPTION_5, FLOOR_ITEM_OPTION_6) {

    override fun decode(ctx: ChannelHandlerContext, packet: Packet): FloorItemOption? {
        return FloorItemOption(packet.readUnsigned(DataType.SHORT, Modifier.ADD).toInt(), packet.readBoolean(), packet.readShort(), packet.readShort(order = Endian.LITTLE), opcodes.indexOf(packet.opcode) + 1)
    }
