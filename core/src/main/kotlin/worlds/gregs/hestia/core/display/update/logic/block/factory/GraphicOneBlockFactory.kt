...
open class GraphicOneBlockFactory(flag: Int, npc: Boolean = false) : BlockFactory<GraphicBlock>(Aspect.all(Renderable::class, FirstGraphic::class), flag = flag, npc = npc) {
    ...
    override fun create(player: Int, other: Int): GraphicBlock {
        val graphic = firstGraphicMapper.get(other)
        return GraphicBlock(flag, 1, graphic.id, (graphic.delay and 0xffff) or (graphic.height shl 16), (graphic.rotation and 0x7) or (/*graphic.slot*/0 shl 3) or (graphic.forceRefresh.int shl 7))
    }
