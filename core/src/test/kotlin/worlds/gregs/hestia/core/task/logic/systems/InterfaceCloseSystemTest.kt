...
internal class InterfaceCloseSystemTest : MockkGame(WorldConfigurationBuilder()) {
    ...
    override fun setup() {
        ...
        tick()
        every { task.suspension = any() } propertyType InterfaceClose::class answers {
            continuation = arg<InterfaceClose>(0).continuation
...
//        println(suspension)
        //When
        es.dispatch(InterfaceClosed(widget, false).apply { entity = entityId })
        //Then
//        verify { tasks.resume(entityId, suspension, Unit) }
    }
