...
import worlds.gregs.hestia.core.scripts.dsl.SystemBuilder.Companion.EMPTY_ASPECT
import worlds.gregs.hestia.core.scripts.dsl.DslPassiveSystem
import worlds.gregs.hestia.core.scripts.dsl.DslSubscriptionSystem
import worlds.gregs.hestia.core.scripts.dsl.DslParallelSystem
import worlds.gregs.hestia.core.scripts.dsl.DslIteratingSystem
 ...
 */
@ArtemisDsl
class EventListenerBuilder<E : Event>(var event: KClass<E>, var priority: Int, var skipCancelledEvents: Boolean, var conditional: ((event: E) -> Boolean)?, var action: ((event: E) -> Unit)?) {
    /**
     * Alternative way of setting event condition
     * @param action The conditions to meet before invoking event action
     */
    fun where(action: (event: E) -> Boolean) {
        if (conditional != null) {
